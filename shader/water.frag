...
float calculateShadowPCF(vec3 projCoords, float receiverDepth, float bias, vec2 texelSize, mat2 rotation, float radius)
{
    ...
    for (int i = 0; i < 16; ++i)
    {
        vec2 offset = rotation * poissonDisk[i] * texelSize * radius;
        float sampleDepth = texture(shadowMap, projCoords.xy + offset).r;
        float weight = 1.0 - saturate(length(poissonDisk[i]) * 0.65);
        shadow += ((receiverDepth - bias) > sampleDepth ? 1.0 : 0.0) * weight;
        weightSum += weight;
    }
...
float calculateShadowPCSS(vec3 projCoords, float receiverDepth, float bias, vec2 texelSize, mat2 rotation)
{
    ...
    float blockers = 0.0;
    float searchRadius = max(shadowLightSize * shadowSoftnessScale * 0.35 * receiverDepth, 1.0) * max(texelSize.x, texelSize.y);
    ...
    float filterRadius = max(1.0, penumbra * shadowSoftnessScale) * shadowLightSize * 850.0 * max(texelSize.x, texelSize.y);
...
float calculateShadow(vec4 fragPosLightSpace, vec3 normal, vec3 lightDir)
{
    ...
    float radius = mix(1.0, 2.2, worldDanger);
...
void main()
{
    ...
    vec3 color = waterColor * ambientStrength;
    color += waterColor * ndotl * 0.24 * sunColor * sunIntensity * (1.0 - shadow);
    color += vec3(0.18, 0.28, 0.34) * fresnel;
    color += vec3(0.90, 0.96, 1.0) * sparkle * sunColor * sunIntensity * (1.0 - shadow);
    color += vec3(0.72, 0.82, 0.88) * foam;
    color += subsurface;
    color *= mix(0.92, 0.72, shadow);
